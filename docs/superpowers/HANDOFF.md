...
## LEAN-BUDGET restructure (human decision, 2026-08-15, session "architecture agent 2") — supersedes the dispatch counts below
Budget: stay **under 30% plan usage** until the whole phase plan ships (baseline 2%); tripwire at ~20% before Phase 3 → cut further. Structure: controller works inline wherever possible; no re-dispatched whole-branch opus review of feature/agent (its findings were lost with the prior session — partial fixes committed at ae2b75b after controller inline review, accepted risk); data collapsed to ~2 implementer dispatches with ONE sonnet diff-review (on the derived+service dispatch only); Phase 3 fully inline; Phase 4 single doc-assembler pass.
Progress this session: **agent workstream CLOSED** (ae2b75b; clarify turns recorded, max_turns ≤3, unmatched-show footnote). **data dispatch (a) COMPLETE**: nasstatus (923cf5a), curated facts YAML + adapter, 32 airports (91f90fb; human decisions: declared_rate_vmc = best-visual-config midpoint / declared_rate_imc = worst-instrument-config midpoint from the 2014-19 FAA profiles; gates omitted everywhere → pax_per_gate & deps_per_gate_day absent). Controller docs follow-ups applied on main (cbf8106; design 03 coverage clause + roster tools; limitations rows 22-27 — worktree branches also add rows numbered from 22, renumber at merge). NOTE: plan2a's RESCOPE addendum (a99e263) is authoritative over the RESCOPE doc: OTP **12 months**, **Census pop kept**, no BEA, no T-100 intl probe. In flight: one sonnet dispatch implementing Socrata + T-100 dom + OTP-12m + Census-pop adapters in ../aa-data. Then: derived+DuckDBDataService+registration+refresh+snapshot dispatch (with sonnet diff-review), then Phase 3.

## RESCOPE adopted (human decision, 2026-08-15) — 8 remaining dispatches
Process: sonnet diff-only reviewers (≤25 lines, Critical/Important only), opus only for one final whole-branch review per workstream; fix rounds only for Critical/Important; controller inspects small fixes; batch adjacent tasks; implementers stay sonnet unless source-format verification is needed. Cuts regardless: Phase-2 live LLM smoke, `feature/voice`, sparkline.
Status: scoring COMPLETE (f5a8c69), ui COMPLETE (e910749). Remaining 8 dispatches: **agent** (a) T7+T8 ✅ (da1ad08, 8d0fd98, 4525caa), (b) T9+T10, (c) T11+T12+T13; **data (Option A Core-6)** (a) NPIAS+curated lists+curated facts YAML+nasstatus, (b) Socrata+T-100 dom, (c) OTP 36m, (d) derived+test snapshot+DuckDBDataService+contract registration, (e) refresh CLI+snapshot+limitations rows. Then Phase 3 (merge data→scoring→agent→ui, wire build_app, run the 4 sample questions live once, /eval-samples) and Phase 4 (doc-assembler, scribe, zip).

## Phase 3 status (2026-08-16 02:15)
Merged to main: data (incl. 33 MB snapshot, vintage ~2026-06/04) → scoring → agent → ui + build_app wiring fix. Gate on main: ruff clean, lint-imports 4/4, 749 passed / 1 skipped / 8 deselected. Live sample-question run through `python -m airport_agent.ui.cli`: wiring verified end-to-end, but the provided GEMINI_API_KEY is quota-exhausted (429 on flash and flash-lite) — loud-failure policy fired as designed. BLOCKED on: fresh key in `.env` (or Google free-tier daily reset, midnight PT). Remaining: live run of the 4 sample questions + follow-ups, then Phase 4 (doc-assembler, scribe, zip).
