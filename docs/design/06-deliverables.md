...
- **The two remaining standalone explanatory docs**
  ...
  - **No `docs/KEY-TRADEOFFS.md`** (revised 2026-08-16, human decision). DESIGN.md's own "Key tradeoffs"
    section is the account; `design/known-limitations-and-tradeoffs.md` remains the full living log behind it.
...
- `docs/DESIGN.md` — the required short design/architecture document (target 4–6 pages + the three embedded
  ...
  3 Scoring methodology (= SCORING-METHODOLOGY.md) · 4 Key tradeoffs (written in DESIGN.md itself) ·
  5 Where/how AI is used (= WHERE-HOW-AI-IS-USED.md) · 6 Assumptions/uncertainty/scoping as product behaviour
  Detailed `docs/design/*` and `docs/research/*` remain as appendices.
...
- DESIGN.md exists and the two standalone docs (SCORING-METHODOLOGY, WHERE-HOW-AI-IS-USED) exist
