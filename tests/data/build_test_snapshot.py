...
def build_test_snapshot(path: Path) -> Path:
    ...
    npias = FaaNpiasAdapter().normalize([FIXTURES_DIR / "faa_npias" / "appendix_a_subset.xlsx"])
    store.replace_rows("npias", npias["npias"], None)

    curated = CuratedFactsAdapter().normalize([FIXTURES_DIR / "curated" / "airport_facts_small.yaml"])
    store.replace_rows("curated_facts", curated["curated_facts"], None)
    store.replace_rows("curated_inputs", curated["curated_inputs"], None)

    socrata = BtsSocrataAdapter().normalize([FIXTURES_DIR / "bts_socrata" / "sample.json"])
    ...
    t100 = BtsT100SegmentAdapter().normalize([FIXTURES_DIR / "bts_t100" / "dom_2026_04_subset.csv"])
    store.replace_rows("routes_month", t100["routes_month"], {"source_id": "bts_t100"})

    otp = BtsOtpAdapter().normalize([FIXTURES_DIR / "bts_otp" / "otp_2026_06_subset.csv"])
    ...
    census = CensusCbsaAdapter().normalize(
        [
